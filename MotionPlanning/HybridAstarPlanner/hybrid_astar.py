...
class C:  # Parameter config
    ...
    # Работаем непосредственно в пикселях (упрощенный подход)
    XY_RESO = 70.0  # [pixels] - разрешение сетки
    YAW_RESO = np.deg2rad(1.0)  # [rad]
    MOVE_STEP = 5.0  # [pixels] - шаг интерполяции пути
    N_STEER = 20.0
    COLLISION_CHECK_STEP = 3
    EXTEND_BOUND = 2

    # Весовые коэффициенты
    GEAR_COST = 50.0
    BACKWARD_COST = 15000000.0
    STEER_CHANGE_COST = 1.0
    STEER_ANGLE_COST = 1.0
    H_COST = 25.0

    # Параметры автомобиля (в пикселях)
    CAR_LENGTH = 60  # [pixels]
    CAR_WIDTH = 30   # [pixels]
    MAX_STEER = math.pi / 6  # [rad] 30 градусов
    
    RF = CAR_LENGTH * 0.6  # [pixels] передний свес (36px)
    RB = CAR_LENGTH * 0.4  # [pixels] задний свес (24px)
    W = CAR_WIDTH          # [pixels] ширина (30px)
    WD = 0.7 * W           # [pixels] колея (21px)
    WB = CAR_LENGTH * 0.8  # [pixels] колесная база (48px)
    TR = 4                 # [pixels] радиус колеса
    TW = 3                 # [pixels] ширина колеса
...
def calc_next_node(n_curr, c_id, u, d, P):
    # Принудительно устанавливаем направление только вперед
    d = 1  # всегда вперед
    ...
    # Штраф за смену направления (маловероятно, но на всякий случай)
...
def analystic_expantion(node, ngoal, P):
    ...
    # Фильтруем пути, оставляя только те, где нет заднего хода
    forward_paths = []
    for path in paths:
        has_backward = any(length < 0 for length in path.lengths)
        if not has_backward:
            forward_paths.append(path)

    # Если нет полностью прямых путей, используем все, но с штрафом
    ...
    for path in forward_paths:
        ...
        # Дополнительный штраф если есть задний ход
        if any(length < 0 for length in path.lengths):
            cost += C.BACKWARD_COST * 0.1  # меньший штраф, но все равно значительный
        pq.put(path, cost)
...
def calc_motion_set():
    # ТОЛЬКО прямое движение с разными углами поворота
    s = np.arange(-C.MAX_STEER, C.MAX_STEER + C.MAX_STEER/C.N_STEER, 
                  C.MAX_STEER / C.N_STEER)
    
    steer = list(s)
    direc = [1.0 for _ in range(len(steer))]  # ВСЕГДА вперед
